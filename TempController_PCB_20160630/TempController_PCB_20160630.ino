...
#define TEC_HIGH_VLIMIT 5
#define TEC_LOW_VLIMIT -5
...
//Define Variables
double Setpoint_A = 21.45, Setpoint_B = 20;
...
RunningAverage touchtime_data(40);
int samples = 0;
int screen = 0, redraw = 0;
//Specify the links and initial tuning parameters
double Kp_A = 1.0, Kp_B = 2.5;
...
//  Serial.println("A0");
//  Serial.println(thermvoltage_A[0]);
//  Serial.println("A1");
//  Serial.println(thermvoltage_A[1]);
//  Serial.println("A2");
//  Serial.println(thermvoltage_A[2]);
//  Serial.println("A3");
//  Serial.println(thermvoltage_A[3]);
//  Serial.println("A5");
//  Serial.println(thermvoltage_A[5]);
  ...
  thermresistance_A = 50000.0 * thermvoltage_A[1] / VRref_A;
  thermresistance_A2 = 50000.0 * thermvoltage_A[3] / VRref_A2;
  thermresistance_A3 = 10000.0 * thermvoltage_A[5] / VRref_A3;

  temperature_A = thermistor50k(thermresistance_A);
  ...
  // Get thermistor resistance from the voltage
  thermresistance_B = 10000.0 * thermvoltage_B[1] / VRref_B;
  thermresistance_B2 = 10000.0 * thermvoltage_B[3] / VRref_B2;
  thermresistance_B3 = 10000.0 * thermvoltage_B[5] / VRref_B3;
  ...
  int dacvoltage_A = 1250 - int(200.0 * Output_A); // Control voltage go ADN8843
  int dacvoltage_B = 1250 - int(200.0 * Output_B); // Control voltage to go to ADN8843
...
void touchaction(void)
{
  if (ctp.touched())  {
    ...
    int INCREASE[4] = {2 + 165, 15, 158, 49};
    int DECREASE[4] = {2 + 165, 15 + 55, 158, 49};
    ...
    if (current_screen == TEC_A_MAIN) {
      if (box_check(coord_x, coord_y, MAIN_HOME)) {
        next_screen = HOME;
      } else if (box_check(coord_x, coord_y, INCREASE)) {

        Setpoint_A = Setpoint_A + increment();

      }
      else if (box_check(coord_x, coord_y, DECREASE)) {
        Setpoint_A = Setpoint_A - increment();
      }
    }
...
void drawvalues(int menu_number) {
  if (menu_number == TEC_A_MAIN) {
    ...
    draw7Number(abs(round(100 * Setpoint_A) % 100), 110, 35, 1, ILI9341_WHITE, ILI9341_BLACK, 2);
    draw7Number(floor(temperature_A), 60, 20 + 55, 2, ILI9341_WHITE, ILI9341_BLACK, -2);
    draw7Number(abs((int)100 * temperature_A - 100 * (int)temperature_A), 110, 35 + 55, 1, ILI9341_WHITE, ILI9341_BLACK, 2);
    //    draw7Number(floor(temperature_A2), 60, 20 + 55 + 55, 2, ILI9341_WHITE, ILI9341_BLACK, -2);
    //    draw7Number(abs((int)100 * temperature_A2 - 100 * (int)temperature_A2), 110, 35 + 55 + 55, 1, ILI9341_WHITE, ILI9341_BLACK, 2);
    draw7Number((int)VTEC_A, 60 + 150, 20, 2, ILI9341_WHITE, ILI9341_BLACK, -2);
    draw7Number(round(VTEC_A * 100.0), 60 + 150 + 50, 35, 1, ILI9341_WHITE, ILI9341_BLACK, 2);
    draw7Number((int)ITEC_A, 60 + 150, 20 + 55, 2, ILI9341_WHITE, ILI9341_BLACK, -2);
    draw7Number(round(ITEC_A * 100.0), 60 + 150 + 50, 35 + 55, 1, ILI9341_WHITE, ILI9341_BLACK, 2);
  }
  ...
  {
    draw7Number((int)Kp_A, 60, 20, 2, ILI9341_WHITE, ILI9341_BLACK, -2);
    draw7Number(round(100 * Kp_A) % 100, 110, 35, 1, ILI9341_WHITE, ILI9341_BLACK, 2);
    draw7Number((int)Ki_A, 60, 20 + 55, 2, ILI9341_WHITE, ILI9341_BLACK, -2);
    draw7Number(round(100 * Ki_A), 110, 35 + 55, 1, ILI9341_WHITE, ILI9341_BLACK, 2);
    draw7Number(int(Kd_A), 60, 20 + 55 + 55, 2, ILI9341_WHITE, ILI9341_BLACK, -2);
    draw7Number(round(100 * Kd_A), 110, 35 + 55 + 55, 1, ILI9341_WHITE, ILI9341_BLACK, 2);
  }
...
{
  touchtime_data.addValue(millis() - touchtime);
  double average_time = touchtime_data.getAverage();
  touchtime = millis();
  if (average_time > 120) {
    return 0.01;
  } else if (average_time <= 120 && average_time > 70) {
    return 0.05;
  } else if (average_time <= 70) {
    return 0.2;
  }
}
