...
int select = 0;
RunningAverage data_A_1(300), data_A_2(300), data_A_1_sink(30), data_A_2_sink(30), data_A_VTEC(20), data_A_ITEC(20); // Put number of samples in the parenthesis
RunningAverage data_B_1(300), data_B_2(300), data_B_1_sink(30), data_B_2_sink(30), data_B_VTEC(20), data_B_ITEC(20); // Put number of samples in the parenthesis
int samples = 0;
...
double Ki_A = 0.1;
double Kd_A = 0;
PID myPID0(&Input_A, &Output_A, &Setpoint_A, Kp_A, Ki_A, Kd_A, DIRECT);
...
void loop()
{
  ...
  Vsense = data_A_VTEC.getAverage();
  Isense = data_A_ITEC.getAverage();
  VTEC = 4.0 * ((3.3 * Vsense / 4096.0) - 1.25);
  ITEC = ((3.3 * Isense / 4096.0) - 1.25) / 0.525;

  thermvoltage[0] = data_A_1.getAverage();
  thermvoltage[1] = data_A_2.getAverage();
  thermvoltage[2] = 4095;
  thermvoltage[3] = data_A_1_sink.getAverage();
  ...
  int dacvoltage = 1250 - 200 * Output_A;
  dac.voutWrite(DAC_PIN_TMON, dacvoltage);
  dac.voutWrite(DAC_PIN_VCON, dacvoltage);
  //Serial.println(data_A_1.getAverage());
  //Serial.println(data_A_2.getAverage());
  if (thermvoltage[0] > 4090)
  {
...
void touchaction2(void)
{
  ...
  myPID0.SetTunings(Kp_A, Ki_A, Kd_A);
}
